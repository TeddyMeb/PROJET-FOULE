...
public class Agent {
    ...
    public void avancer_avec_Carte(Carte carte){
        ...
        double angleAleatoire = direction + (Math.random() - 0.5) * Math.PI ; // tourner jusqu’à 90° à gauche ou droite
        Point capAlternatif = new Point(Math.cos(angleAleatoire), Math.sin(angleAleatoire));
        Point nouvellePos = new Point(position.getX() + vmax * capAlternatif.getX(), position.getY() + vmax * capAlternatif.getY());
        if (carte.estAccessible(nouvellePos)) {
            cap = capAlternatif;
    ...
    public void mettreAJourDirection2(Carte carte, ArrayList<Agent> voisins, double longueurRayon, int k, double lambda) {
        int compteurBloque=0;
        double meilleurAngle = choisirNouvelleDirection(voisins, longueurRayon, k);
        if (!peutAvancerDirection(meilleurAngle, carte, voisins)) {
            compteurBloque++;
        if (compteurBloque > 5) {
            // Forcer la rotation pour débloquer
            meilleurAngle += Math.PI / 2; // tourner de 90°
            compteurBloque = 0;
        }
        } else {
            compteurBloque = 0;
    ...
    public boolean peutAvancerDirection(double angle, Carte carte, ArrayList<Agent> voisins) {
