...
public class SimulationAvecFenetreGraphique {
    ...
    public void lancerSimulation(int mode, double longueurRayon, int nombreRayons,double lambda) {
        simulationThread = new Thread(() -> {
            try {
                switch (mode) {
                    case 1:
                        simulationMode1et2(mode, lambda, nombreRayons, longueurRayon);
                        return;
                    case 2:
                        simulationMode1et2(mode, lambda, nombreRayons, longueurRayon);
                        return;
                    case 3:
                        simulationMode3(lambda, nombreRayons, longueurRayon);
                        return;
                    case 4:
    ...
    private void simulationMode1et2(int mode, double longueurRayon, int nombreRayons, double lambda) throws InterruptedException {
        ...
        if (mode == 1) {
            ...
            foule2.positionAgentEnGrille2(copieAgentFoule1, 10, 10, 480, 50, 30);
        }
        ...
        if (mode==1){
            while (!foule1.getAgents().isEmpty() || !foule2.getAgents().isEmpty()) {
                ...
                while (i < foule1.getAgents().size()) {
                    ...
                    copieAgents.get(i).mettreAJourDirection(concatener,longueurRayon, nombreRayons, lambda);
                ...
                while (i2 < foule2.getAgents().size()) {
                    ...
                    copieAgents2.get(i2).mettreAJourDirection(concatener2,longueurRayon, nombreRayons, lambda);
        ...
        if (mode == 2){
            ...
            while (agentsArrives1.size() < foule1.getAgents().size() || agentsArrives2.size() < foule2.getAgents().size() ) {
                ...
                fenetre.effacer();
                for (Agent agent : foule1.getAgents()) {
                    agent.mettreAJourDirection(foule1.getAgents(),longueurRayon, nombreRayons, lambda);
                    agent.avancer();
    ...
    private void simulationMode3(double longueurRayon, int nombreRayons,double lambda) throws InterruptedException {
        Point sortie = new Point(400, 300);
        Foule foule1 = new Foule(new ArrayList<>(), sortie);
        foule1.positionAgentEnCercle(100, sortie,250);
        ...
        while (!foule1.getAgents().isEmpty() || !foule2.getAgents().isEmpty() || !foule3.getAgents().isEmpty()) {
            ...
            Consumer<Foule> avancerFoule = (foule) -> {
                ...
                for (Agent agent : agents) {
                    ...
                    agent.mettreAJourDirection2(carte, tousLesAgents,longueurRayon, nombreRayons, lambda);
