...
public class Foule {
    ...
    public void retirerAgents(Agent agent){
     ...
     public void positionAgentAleatoire(int n){
         ...
         for (int k =0;k<n;k++){
            double coordonnees_x=50+Math.random()*1400;
            double coordonnees_y=50+Math.random()*900;
            Agent agent = new Agent(new Point(coordonnees_x,coordonnees_y),sortie,8,1,Color.BLUE);
    ...
    public void positionAgentEnGrille(int lignes, int colonnes, double x_depart, double y_depart, double espace){
        ...
        for (int i=0; i<lignes; i++) {
            for (int j=0;j<colonnes; j++) {
                ...
                Agent agent = new Agent(new Point(x,y),this.sortie,8,1,Color.BLUE);
            agents.add(agent);
            }
        }
    }
    ...
    public void positionAgentEnCercle(int nbAgents, Point centre, double rayon){
        ...
        for (int i=0; i<nbAgents; i++){
            ...
            Agent agent = new Agent(position,objectif,8,1,Color.RED);
            agents.add(agent);    
        }
    }
}
